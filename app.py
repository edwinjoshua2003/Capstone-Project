...
all_symp_pr = [BioBERT_Model(sym) for sym in all_symp]
...
def semanticD(doc1, doc2):
    doc1_p = BioBERT_Model(doc1).split(' ')
    doc2_p = BioBERT_Model(doc2).split(' ')
    score = 0
    for tock1 in doc1_p:
        for tock2 in doc2_p:
            syn1 = WSD(tock1, doc1)
...
def get_bot_response():
    ...
    if session['step'] == "Severity":
        ...
        if calc_condition(session["all"], int(s)) == 1:
            return "you should take the consultation from doctor <br> Tap q to check for any suggested drugs"
        else:
            ...
            for e in precautionDictionary[session["disease"]]:
                msg += '\n ' + str(i) + ' - ' + e + '<br>'
                i += 1
            msg += ' Tap q to check for any suggested drugs'
            return msg
        
    if session['step'] == "FINAL":
        session['step'] = "SUGGEST_DRUGS"
        suggested_drugs = suggest_drugs(session["disease"])  # Get suggested drugs based on symptoms
    ...
    if session['step'] == "BYE":
        ...
        if s.lower() == "yes":
            ...
            return "THANKS Mr/Ms " + name + " for using me, for feedback please contact <b> edwinjoshua2003@gmail.com</b>"
